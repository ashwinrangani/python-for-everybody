...
row = cursor.fetchone()
if row is not None:
    print("Restarting existing crawl, Remove spider.sqlite to start fresh crawl")
else:
...
while True:
    ...
    # If we are retrieving this page, there should be no links
    ...
    try:
        ...
        if document.getcode() != 200:
            print('Error on page: ', document.getcode(), url)
            cursor.execute('UPDATE Pages SET error=? WHERE=?', (document.getcode(), url) )
